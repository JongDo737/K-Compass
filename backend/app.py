...
def initial_recommend_restaurant():
    ...
    print(age,gender,companion,requirements,travel_period)
    return "200"
...
def initial_recommend_restaurant(age, gender, companion, requirements, travel_period, restaurant_data, offset=0):
    ...
    user_input = input("추천된 맛집 중 하나를 선택해주세요 (1, 2, 3). 더 많은 옵션을 원하시면 '###'를 입력해주세요.\nPlease choose one of the recommended restaurants (1, 2, 3) or enter '###' for more options: ")
    if user_input == '###':
        offset += 3
        return initial_recommend_restaurant(age, gender, companion, requirements, travel_period, restaurant_data, offset)
